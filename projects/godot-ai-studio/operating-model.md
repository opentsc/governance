...
- **他人 PR 处置纪律（2026-08-08 负责人裁决）**：实质性问题（设计/逻辑/结构）
  一律打回（request changes + 具体整改路径），禁止代改。唯一的例外是
  **门禁级机械修复**（lint/格式/注解/CI 配置，不含任何实现语义），且必须
  满足三个条件：① 作者超时未响应或明确授权；② 修复内容在 PR 评论里逐条
  披露；③ 合并前 CI 全绿。凡越界的代改视为流程事故处理。
